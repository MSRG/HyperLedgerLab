...
async function runFixedNumber(msg, cb, context) {
	...
	if (msg.label == 'initLedgerGen' || msg.label == 'commonGen') {
		//logger.debug('INSIDE IF initLedgerGen');
		promises.push(cb.run(msg.clientIdx).then((result) => {
	        	addResult(result);
		...
		//logger.debug('INSIDE ELSE initLedgerGen');
	...
	//logger.debug('OUTSIDE IF AND ELSE');
    }
...
async function runDuration(msg, cb, context) {
	...
	if (msg.label == 'initLedgerGen' || msg.label == 'commonGen') {
                //logger.debug('INSIDE IF commonGen');
	        ...
	        await rateControl.applyRateControl(startTime, txNum, results, resultStats);
                ...
                //logger.debug('INSIDE ELSE commonGen');
		promises.push(cb.run().then((result) => {
                            addResult(result);
                            return Promise.resolve();
                }));
