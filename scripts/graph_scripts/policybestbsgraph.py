...
f1.write('EndorsePolicy,failTxPercent,TotMVCCPercent,MVCCInterPercent,MVCCIntraPercent,PhantomReadPercent,EndorseFailBCPercent,AvgSuccLat,\n')
...
for tps in dfgraph['EndorsePolicy'].unique():
	f1.write(repr(tps))
	f1.write(',')
        for y in yvalues:
		dffilter = dfgraph[dfgraph['EndorsePolicy'] == tps]
		#print(dffilter)
...
f1.write('EndorsePolicy,succTxPercent,CommThrput,SuccThrput,\n')
...
for y in yvalues:
	plotname = 'BestBlockSizeVsEndorsePolicy_Min'+ str(y)
	plt.bar(dfgraph['EndorsePolicy'], dfgraph[y], width=0.5, color='blue', edgecolor='blue', ecolor='black')
	plt.xlabel("EndorsePolicy")
	plt.ylabel("BlockSize")
	plt.xticks(dfgraph['EndorsePolicy'])
	print(dfgraph['EndorsePolicy'])
	plt.title(plotname)
	plt.legend()
        plt.ylim(bottom=0)
        #plt.xlim(0, len(dfgraph['TPS']))
	print(len(dfgraph['EndorsePolicy']))
...
for y in yvalues:
        plotname = 'BestBlockSizeVsEndorsePolicy_Max'+ str(y)
        plt.bar(dfgraph['EndorsePolicy'], dfgraph[y], width=0.5, color='blue', edgecolor='blue', ecolor='black')
        plt.xlabel("EndorsePolicy")
        plt.ylabel("BlockSize")
        plt.xticks(dfgraph['EndorsePolicy'])
