...
for tps in dfgraph['Database'].unique():
	for y in yvalues: 
		...
		plotname = 'BlockSize_Vs_'+ str(y) + '_at_' + str(int(tps)) + 'Database'
		...
		try:
			os.makedirs("./%s/PerDatabase/%d" % (ExpName, tps))
		except OSError as err:
	   		print(err)
		plt.savefig('./%s/PerDatabase/%d/%s.png' % (ExpName, tps, plotname))
		plt.close()
...
for bs in dfgraph['BlockSize']:
        for y in yvalues:
                ...
                dffilter = dffilter.reset_index(drop=True)
                plotname = 'Database_Vs_'+ str(y) + '_at_' + str(int(bs)) + '_Blocksize'
                plt.bar(dffilter['Database'], dffilter[y], yerr=dffilter[y+'SE'], width=0.5, color='blue', edgecolor='blue', ecolor='black')
                plt.xlabel("Database")
                plt.ylabel(y)
                plt.title(plotname)
                plt.legend()
