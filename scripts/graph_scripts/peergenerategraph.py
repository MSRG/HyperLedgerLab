...
for tps in dfgraph['PeersPerOrg'].unique():
	for y in yvalues: 
		...
		plotname = 'BlockSize_Vs_'+ str(y) + '_at_' + str(int(tps)) + 'PeersPerOrg'
		...
		try:
			os.makedirs("./%s/PerPeersPerOrg/%d" % (ExpName, tps))
		except OSError as err:
	   		print(err)
		plt.savefig('./%s/PerPeersPerOrg/%d/%s.png' % (ExpName, tps, plotname))
		plt.close()
...
for bs in dfgraph['BlockSize'].unique():
        for y in yvalues:
                ...
                dffilter = dffilter.reset_index(drop=True)
                plotname = 'PeersPerOrg_Vs_'+ str(y) + '_at_' + str(int(bs)) + '_Blocksize'
                plt.bar(dffilter['PeersPerOrg'], dffilter[y], yerr=dffilter[y+'SE'], width=0.5, color='blue', edgecolor='blue', ecolor='black')
