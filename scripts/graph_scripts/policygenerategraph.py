...
for tps in dfgraph['EndorsePolicy'].unique():
	for y in yvalues: 
		...
		plotname = 'BlockSize_Vs_'+ str(y) + '_at_' + str(int(tps)) + 'EndorsePolicy'
		...
		try:
			os.makedirs("./%s/PerEndorsePolicy/%d" % (ExpName, tps))
		except OSError as err:
	   		print(err)
		plt.savefig('./%s/PerEndorsePolicy/%d/%s.png' % (ExpName, tps, plotname))
		plt.close()
...
for bs in dfgraph['BlockSize'].unique():
        for y in yvalues:
                ...
                dffilter = dffilter.reset_index(drop=True)
                plotname = 'EndorsePolicy_Vs_'+ str(y) + '_at_' + str(int(bs)) + '_Blocksize'
                plt.bar(dffilter['EndorsePolicy'], dffilter[y], yerr=dffilter[y+'SE'], width=0.5, color='blue', edgecolor='blue', ecolor='black')
