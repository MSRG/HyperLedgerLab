...
f1.write('PeersPerOrg,failTxPercent,TotMVCCPercent,MVCCInterPercent,MVCCIntraPercent,PhantomReadPercent,EndorseFailBCPercent,AvgSuccLat,\n')
...
for tps in dfgraph['PeersPerOrg'].unique():
	f1.write(repr(tps))
	f1.write(',')
        for y in yvalues:
		dffilter = dfgraph[dfgraph['PeersPerOrg'] == tps]
		#print(dffilter)
...
f1.write('PeersPerOrg,succTxPercent,CommThrput,SuccThrput,\n')
...
for y in yvalues:
	plotname = 'BestBlockSizeVsPeersPerOrg_Min'+ str(y)
	plt.bar(dfgraph['PeersPerOrg'], dfgraph[y], width=0.5, color='blue', edgecolor='blue', ecolor='black')
	plt.xlabel("PeersPerOrg")
	plt.ylabel("BlockSize")
	plt.xticks(dfgraph['PeersPerOrg'])
	print(dfgraph['PeersPerOrg'])
	plt.title(plotname)
	plt.legend()
        plt.ylim(bottom=0)
        #plt.xlim(0, len(dfgraph['TPS']))
	print(len(dfgraph['PeersPerOrg']))
...
for y in yvalues:
        plotname = 'BestBlockSizeVsPeersPerOrg_Max'+ str(y)
        plt.bar(dfgraph['PeersPerOrg'], dfgraph[y], width=0.5, color='blue', edgecolor='blue', ecolor='black')
        plt.xlabel("PeersPerOrg")
        plt.ylabel("BlockSize")
        plt.xticks(dfgraph['PeersPerOrg'])
